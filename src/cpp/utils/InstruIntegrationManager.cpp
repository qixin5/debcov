...
InstruIntegrationManager *InstruIntegrationManager::Instance;
...
void InstruIntegrationManager::capture() {
  ...
  std::string ChiselOutputDir = "CHISEL_OUTPUT_DIR=" + InstruOptionManager::OutputDir;
  ...
  std::vector<llvm::StringRef> Args;
  for (auto &Arg : InstruOptionManager::BuildCmd)
    Args.push_back(Arg);
  ...
  llvm::ErrorOr<std::string> Program = llvm::sys::findProgramByName(
      InstruOptionManager::BuildCmd[0], llvm::ArrayRef<llvm::StringRef>(PathVecRef));
  if (Program) {
    // Clean up the repository (e.g., make clean). Delete files that were
    // generated by the initial running of the test script in InstruOptionManager
    clean(Program.get());
    std::string Msg;
    ...
    llvm::errs() << "Error: Unsupported build system "
                 << InstruOptionManager::BuildCmd[0] << "\n";
    exit(1);
  }
}

void InstruIntegrationManager::buildCompilationDB() {
  std::string Line;
  ...
  for (auto &E : CompilationDB)
    InstruOptionManager::InputFiles.push_back(E.first);
}

void InstruIntegrationManager::Initialize() {
  Instance = new InstruIntegrationManager();
  Instance->CaptureFilePath = InstruOptionManager::OutputDir + "/capture.txt";
  Instance->WrapperDir =
      InstruFileManager::Dirname(InstruOptionManager::BinFile) + "/../lib/wrappers";
  spdlog::get("Logger")->debug("Wrapper Directory: {}", Instance->WrapperDir);
  if (InstruOptionManager::Build) {
...
InstruIntegrationManager::getCC1Args(std::string &FileName) {
...
void InstruIntegrationManager::Finalize() {
  //for (auto &File : InstruOptionManager::InputFiles) {
  //  std::string Origin = File + ".origin.c";
  //  llvm::sys::fs::copy_file(File, File + ".instru.c");
