...
void InstruTransformation::Initialize(clang::ASTContext &C) {
  Context = &C;
  TheRewriter.setSourceMgr(Context->getSourceManager(), Context->getLangOpts());
}

std::vector<clang::Stmt *> InstruTransformation::getAllChildren(clang::Stmt *S) {
...
std::vector<clang::Stmt *> InstruTransformation::getAllPrimitiveChildrenStmts(clang::CompoundStmt *S) {
  std::queue<clang::Stmt *> ToVisit;
...
clang::Rewriter& InstruTransformation::getTheRewriter() {
  return TheRewriter;
}
