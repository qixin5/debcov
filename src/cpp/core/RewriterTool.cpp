...
bool RewriterTool::InsertTextBefore(SourceLocation sl, std::string txt) {
  //Lexer::isAtStartOfMacroExpansion(sl, theSM, theLO, &sl); //Got error, don't know why...
...
std::string RewriterTool::GetSourceText(clang::SourceLocation start, clang::SourceLocation end) const {
...
void RewriterTool::EmitError(SourceLocation sl, std::string msg) {
  llvm::errs() << theSM.getFilename(sl).str() << ":"
               << theSM.getSpellingLineNumber(sl) << ":"
...
clang::SourceManager& RewriterTool::GetSourceManager() const {
  return theSM;
}

const clang::LangOptions& RewriterTool::GetLangOptions() const {
  return theLO;
}
